...
export default function LiveMatchCard({ match }) {
  const { homeTeam, awayTeam, teamScore, round, singlesMatches, status } = match

  return (
    <Link
      to={`/match/${match.id}`}
      className="block bg-court-card rounded-xl border border-white/5 hover:border-court-accent/30 transition-all hover:shadow-lg hover:shadow-court-accent/5"
    >
      {/* Header */}
